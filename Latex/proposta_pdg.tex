...
A DTW(\textit{Dynamic Time Warping})\cite{DTW} aparece como uma excelente solução para os dois problemas, pois produz como resultado uma matriz de alinhamento, da qual é possível extrair informações sobre a semelhança entre as matrizes \textbf{\textit{H}}. Assim, em um primeiro momento, ela é utilizada para selecionar o semitom correto (os outros candidatos são descartados). Depois, extrai-se dela algumas \textit{features} que serão passadas a um classificador \textit{Random Forest}. Este último algoritmo se faz necessário, pois a probabilidade final do \textit{sample} estar presente na música alvo não pode ser definida a partir de uma única característica da matriz de alinhamento, exigindo uma complexidade maior no momento da classificação (presente ou ausente). 
