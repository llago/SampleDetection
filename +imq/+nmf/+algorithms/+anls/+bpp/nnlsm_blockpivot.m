...
function [ X,Y,success,numChol,numEq ] = nnlsm_blockpivot( A, B, isInputProd, init )
    ...
    if nargin<4
        ...
        PassiveSet = (init > 0);
        [ X,numChol,numEq] = imq.nmf.algorithms.anls.bpp.normalEqComb(AtA,AtB,PassiveSet);
        Y = AtA * X - AtB;
    end
    % parameters
    pbar = 3;
    ...
    while(~isempty(find(NotOptCols)))
        ...
        [ X(:,NotOptCols),tempChol,tempEq ] = imq.nmf.algorithms.anls.bpp.normalEqComb(AtA,AtB(:,NotOptCols),PassiveSet(:,NotOptCols));
