function [part, L, D, W, H] = init_ntfclus_mu_kl_mag (X, K, nsrc, init_rnd, init_iterations, after_iterations)

    %
    % [L, D, W, H] = init_ntfclus_mu_kl_mag(X, K);
    %
    ...
    V = abs(X) + randn(F, N, nchann) * small_noise_var;
    ...
    for init_ind = 1:init_rnd
        ...
        [D, W, H, E] = ntfclus_mu_kl(V, init_iterations, L, D_ini, W_ini, H_ini);
        Q = D * L;
    ...
    if after_iterations == 0,
        ...
        [D, W, H, E] = ntfclus_mu_kl(V, after_iterations, L, D_best, W_best, H_best);
    end
