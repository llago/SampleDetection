function [W, H] = init_nmf_is_em_pow (X, K, init_rnd, init_iterations, after_iterations)

    %
    % [W, H] = init_nmf_is_em_pow
    %
    ...
    V = randn(F, N).^2 * small_noise_var;
    for chann = 1:n_c, V = V + abs(X(:,:,chann)).^2; end;
    ...
    for init_ind = 1:init_rnd
        ...
        [W,H,E] = is_nmf_em(V,W,H,init_iterations);
    ...
    if after_iterations == 0,
        ...
        [W, H] = is_nmf_em(V,W_best,H_best,after_iterations);
    end
