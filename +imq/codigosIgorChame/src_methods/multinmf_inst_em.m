...
if I ~= 2
    error('Multi_NMF_EM_inst: number of channels must be 2');
end;
...
% MAIN LOOP
h = waitbar(0,'EM/NTFclus-IS');
for iter = 1:iter_num, waitbar(iter/iter_num,h)
    ...
    cost(iter) = trace( squeeze(mean(mean(Sigma_x,1),2)) * inv(squeeze(mean(bar_Rxx,1))) ) - log(det( squeeze(mean(mean(Sigma_x,1),2)) * inv(squeeze(mean(bar_Rxx,1))) )) - 2;
