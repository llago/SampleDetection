...
function [SDRi,ISRi,SIRi,SARi,permi,cost] = nmf_inst_em_is ( NMF_CompPerSrcNum, nsrc, stft_win_len, max_iterations, mixture_full_path, source_ref_full_path, dataset_name, experiment_name, init_param)

    %
    % nmf_inst_em_is
    %
    % Example of usage of EM rules for singlechannel NMF decomposition in
    ...
    for repeat=1:repeat_total,
        ...
        [W_init, H_init] = init_nmf_is_em_pow(X, K, init_rnd, init_iterations, after_iterations);
        ...
        tic;
        [W, H, cost(repeat,:)] = is_nmf_em(V, W_init, H_init, max_iterations);
