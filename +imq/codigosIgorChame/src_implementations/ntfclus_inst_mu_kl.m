...
function [SDRi,ISRi,SIRi,SARi,permi,cost] = ntfclus_inst_mu_kl ( NMF_CompPerSrcNum, nsrc, stft_win_len, max_iterations, mixture_full_path, source_ref_full_path, dataset_name, experiment_name, init_param)
    ...
    for repeat=1:repeat_total,
        ...
        [source_NMF_ind, L, D_init, W_init, H_init] = init_ntfclus_mu_kl_mag(X, K, nsrc, init_rnd, init_iterations, after_iterations);
        ...
        tic;
        [D, W, H, cost(repeat,:)] = ntfclus_mu_kl(V, max_iterations, L, D_init, W_init, H_init);
        Q = D * L;
    ...
    W = W_best;
    H = H_best;
    clear D_init W_init H_init Q_best W_best H_best L_best source_NMF_ind_best;
    ...
    for j=1:nsrc,
        ...
        S_ref_STFT_power(j,:,:,:) = reshape( (abs(S_ref_STFT)), 1, nbin, nfram, nchann);
    end;
