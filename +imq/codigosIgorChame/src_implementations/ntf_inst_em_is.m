...
function [SDRi,ISRi,SIRi,SARi,permi,cost] = ntf_inst_em_is ( NMF_CompPerSrcNum, nsrc, stft_win_len, max_iterations, mixture_full_path, source_ref_full_path, dataset_name, experiment_name, init_param)
    ...
    for repeat=1:repeat_total,
        ...
        [ source_NMF_ind, L, D_init, W_init, H_init, Sigma_b_init] = init_ntf_em_is_pow(X, K, nsrc, init_rnd, init_iterations, after_iterations);
        ...
        tic; 
        [W, H, D, Sigma_b, S_est, cost(repeat,:)] = multinmf_inst_em(X, W_init, H_init, D_init, Sigma_b_init, source_NMF_ind, max_iterations);
        Q = D * L;
        elapsedTime = toc;  LOG.info('experiment',['  Time elapsed Optimization: ' num2str(elapsedTime,'%9.3f') 's']);
        LOG.info('experiment',['  Log-likelihood: ' num2str(cost(repeat,end),'%9.2f')]);
    
        if ( cost_best == 0 ) || ( cost(repeat,end) > cost_best(1,end) )
            LOG.info('experiment','  !New best!');
            source_NMF_ind_best = source_NMF_ind;
            L_best = L;
            Q_best = Q;
            D_best = D;
            W_best = W;
    ...
    source_NMF_ind = source_NMF_ind_best;
    L = L_best;
    Q = Q_best;
    D = D_best;
    W = W_best;
    H = H_best;
    S_est_EM = S_est_best;
    Sigma_b = Sigma_b_best;
    clear D_init W_init H_init Q_best W_best H_best L_best source_NMF_ind_best Sigma_b_best S_est_best D_best;
    h = figure; semilogy(1:max_iterations,cost); title('Divergência de Itakura-Saito Negativa'); ylabel('$C(\cdot)$','Interpreter','latex'); xlabel('Iterations'); grid on;
    ...
    % S_est_STFT=src_image(S_est_EM,D);
