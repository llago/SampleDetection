...
function [SDRi,ISRi,SIRi,SARi,permi,cost] = nmf_inst_mu_is ( NMF_CompPerSrcNum, nsrc, stft_win_len, max_iterations, mixture_full_path, source_ref_full_path, dataset_name, experiment_name, init_param)
    ...
    % Setting spath to sources references
    ...
    for repeat=1:repeat_total,
        ...
        [W_init, H_init] = init_nmf_kl_mu_pow(X, K, init_rnd, init_iterations, after_iterations);
        ...
        tic;
        [W, H, cost(repeat,:)] = is_nmf_mu(V, W_init, H_init, max_iterations);
    ...
    tic;
    [source_NMF_ind] = assoc_compToSrc_ideal(W,H,S_ref_STFT_power);
