...
function [SDRi,ISRi,SIRi,SARi,permi,cost] = nmf_inst_mu_kl ( NMF_CompPerSrcNum, nsrc, stft_win_len, max_iterations, mixture_full_path, source_ref_full_path, dataset_name, experiment_name, init_param)

    %
    % nmf_inst_mu_kl
    %
    % Example of usage of MU rules for singlechannel NMF decomposition in
    ...
    %x = x(:,1); % Selecting only the first channel since we are analysing a one channel approach
  s
    nchann = size(x,2);
    ...
    V = abs(X); % Magnitude of mixture STFT
    ...
    for repeat=1:repeat_total,
        ...
        tic;
        [W_init, H_init] = init_nmf_kl_mu_mag(X, K, init_rnd, init_iterations, after_iterations);
        ...
        tic;
        [W, H, cost(repeat,:)] = nmf_kl_mu(V, W_init, H_init, max_iterations);
        elapsedTime = toc;  LOG.info('experiment',['  Time elapsed Optimization: ' num2str(elapsedTime,'%9.3f') 's']);
        LOG.info('experiment',['  Min cost: ' num2str(cost(repeat,end),'%10.8f')]);
    ...
    W = W_best;
    H = H_best;
    clear W_init H_init W_best H_best;
    h = figure; semilogy(1:max_iterations,cost); title('Divergencia de Kullback-Leibler Generalizada'); ylabel('$C(\cdot)$','Interpreter','latex'); xlabel('Iterations'); grid on;
    ...
    S_ref_STFT_mag = zeros(nsrc,nbin,nfram,nchann);
    for j=1:nsrc,
        s_ref_temp = audioread(source_ref_full_path{j});
        % very small normal noise to avoid zeros
        for chan=1:nchann, if min(abs(s_ref_temp(:,chan))) == 0,   eps_noise = randn(size(s_ref_temp(:,chan),1),1); s_ref_temp(:,chan) = s_ref_temp(:,chan) + eps*(eps_noise ./ max(abs(eps_noise)));  end;  end;
        s_ref(j,:,:)=reshape(s_ref_temp(1:mix_nsamp,1:nchann),1,mix_nsamp,nchann);
        % Guarantees normalization of channels
        % for chann = 1:size(s_ref,3), s_ref(j,:,chann) = s_ref(j,:,chann) ./ max(abs(s_ref(j,:,chann))); end;
        S_ref_STFT = stft_multi(s_ref_temp(1:mix_nsamp,1:nchann).',stft_win_len);
        S_ref_STFT_mag(j,:,:,:) = reshape( (abs(S_ref_STFT)), 1, nbin, nfram, nchann);
    end;
    ...
    tic;
    [source_NMF_ind] = assoc_compToSrc_ideal(W,H,S_ref_STFT_mag);
    ...
    tic;
    S_est_STFT = nmf_recons_im(X,W,H,source_NMF_ind);
    ...
    clear X x s_ref S_ref_STFT_mag LOG V chann plot_ind j k f; % clear some information to reduce file size
