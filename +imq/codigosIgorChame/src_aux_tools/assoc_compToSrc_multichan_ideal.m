function [source_NMF_ind] = assoc_compToSrc_multichan_ideal(Q,W,H,S_ref_STFT_V)

    % Ideal association between components and sources by
    % minimizing V distances
    %
    % [source_NMF_ind] = assoc_compToSrc_multichan_ideal(Q,W,H,S_ref_STFT_V);
    %
    % Inputs:
    % ------
    %
    ...
    V = zeros(ncmp,nbin_est,nfram_est);
    S_ref_STFT_V_eq = zeros(nsrc,nbin,nfram);
    for i=1:nchann, 
        for k=1:ncmp, 
            V(k,:,:) = V(k,:,:) + reshape(Q(i,k) .* ( W(:,k) * H(k,:) ),1,nbin_est,nfram); 
        end;
        S_ref_STFT_V_eq(:,:,:) = S_ref_STFT_V_eq(:,:,:) + S_ref_STFT_V(:,:,:,nchann);
    end;
    ...
    for k=1:ncmp,
        for j=1:nsrc,    
            similarity(k,j) = sum(sum( (S_ref_STFT_V_eq(j,:,:)).^2 )) ./ ( sum(sum( (S_ref_STFT_V_eq(j,:,:) - V(k,:,:)).^2 )) ); 
        end;
    end;
