function [X,x2]=STFT(x,nroPtsFFT,L,S,w,faseZero)
...
X=zeros(nroPtsFFT,F);
x=x(:);
x2=[zeros(F2*S,1);x;zeros(L+(F1-1)*S-T,1)];%Completa o sinal com zeros para ser coerente com as medidas da janela e o numero de frames coerentes com F1 e F2

if (faseZero)
    
        for f=(0:F-1)
    
        x_janelado=x2((1:L)+f*S).*w;
